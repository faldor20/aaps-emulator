import { generate } from './iob/index.js';
...
describe('IOB Calculation Tests', () => {
  ...
  it('should calculate IOB differently with different insulin curve', () => {
    const inputs = {
      clock: new Date('2023-04-15T12:00:00Z').toISOString(),
      profile: {
        dia: 5,
    ...
    const new_inputs = JSON.parse(JSON.stringify(inputs));
    new_inputs.profile = {
      ...inputs.profile,
      curve: "ultra-rapid",
      dia: 9,
    ...
    const new_result = generate(new_inputs, false, null);
    console.log("result", result[10]);
    expect(result[10].iob).to.greaterThan(new_result[10].iob);

    console.log(result[10].iob, new_result[10].iob, "difference:", result[10].iob - new_result[10].iob);
