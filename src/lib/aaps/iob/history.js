...
import * as basalprofile from '../profile/basal.js';
...
/**
 * 
 * @param {{started_at: Date, duration: number}} event 
 * @param {any} splitter 
 * @returns 
 */
function splitTimespanWithOneSplitter(event,splitter) {
    ...
    if (splitter.type === 'recurring') {
        ...
        if (new Date(event.started_at) == new Date()) {
            throw new Error("event.started_at is a empty");
        }
        var startMinutes = new Date(event.started_at).getHours() * 60 + new Date(event.started_at).getMinutes();
...
function calcTempTreatments (inputs, zeroTempDuration) {
    ...
    var firstResumeTime = new Date();
    var lastSuspendTime = new Date();
    ...
    for (i=0; i < pumpHistory.length; i++) {
        ...
        if (current.bolus && current.bolus._type === "Bolus") {
            //TODO: eli this is nonsensical, remove it
            var temp = current;
            current = temp.bolus;
        }
    ...
    tempHistory.forEach(function splitEvent(o) {
    ...
    if (suspend_zeros_iob) {
        ...
        splitHistoryByBasal.forEach(function splitSuspendEvent(o) {
        ...
        pumpSuspends.forEach(function createTempBasal(o) {
            var zTempBasal = [{
                _type: 'SuspendBasal',
                rate: 0,
                duration: o.duration,
        ...
        zTempSuspendBasals.forEach(function splitEvent(o) {
