...
import round_basal  from './round-basal.mjs'
...
export function convert_bg(value, profile)
{
...
{   var msg = '';
    ...
    if ( liftISF < minISFReduction ) {
        addMsg("weakest autoISF factor", round(liftISF,2), "limited by autoISF_min", minISFReduction);
        liftISF = minISFReduction;
    } else if ( liftISF > maxISFReduction ) {
        addMsg("strongest autoISF factor", round(liftISF,2), "limited by autoISF_max", maxISFReduction);
    ...
    addMsg("final ISF factor is", round(final_ISF,2), origin_sens);
    console.error("----------------------------------");
    console.error("end autoISF");
    console.error("----------------------------------");
    return {final_ISF,msg};
}
...
{  
    ...
    if ( !profile.enable_autoISF ) {
        addMsg("autoISF disabled in Preferences");
        console.error("----------------------------------");
        console.error("end autoISF");
        console.error("----------------------------------");
        return {sens,msg};
    ...
    var minmax_delta = null;       // back from 5min block to 1 min
    var minmax_value = null;
    let parabola_below_target = false;
    let parabola_fit_ok = true;
    if (glucose_status.parabola_fit_a2 !=0 && fit_corr>=0.9) {
         minmax_delta = - glucose_status.parabola_fit_a1/2/glucose_status.parabola_fit_a2 * 5;       // back from 5min block to 1 min
        ...
        if (minmax_delta>0 && bg_acce<0) {
            addMsg("Parabolic fit extrapolates a maximum of", convert_bg(minmax_value,profile), "in about", minmax_delta, "minutes");
        } else if (minmax_delta>0 && bg_acce>0) {
            addMsg("Parabolic fit extrapolates a minimum of", convert_bg(minmax_value,profile), "in about", minmax_delta, "minutes");
            if (minmax_delta<=30 && minmax_value<target_bg) {   // start braking
                acce_weight = -profile.bgBrake_ISF_weight;
                parabola_below_target = true;
                addMsg("extrapolation below target soon: use bgBrake_ISF_weight instead");
            }
        }
    }
    if ( fit_corr<0.9 ) {
        addMsg("acce_ISF adaptation by-passed as correlation", round(fit_corr,3), "is too low");
        parabola_fit_ok = false;
    } else {
        ...
        addMsg("acce_ISF adaptation is", round(acce_ISF,2));
        if ( acce_ISF != 1 ) {
           sens_modified = true;
        }
    }
    ...
    addMsg("bg_ISF adaptation is", round(bg_ISF,2));
    var liftISF = 1;
    var final_ISF = 1;
    if (bg_ISF<1) {
        ...
        if ( acce_ISF>1 ) {
             ...
             addMsg("bg_ISF adaptation lifted to", round(liftISF,2), "as bg accelerates already");
        }
        const {final_ISF,msg:finalISF_msg} = withinISFlimits(liftISF, profile.autoISF_min, maxISFReduction, sensitivityRatio, origin_sens, profile, high_temptarget_raises_sensitivity, target_bg, normalTarget, stepActivityDetected, stepInactivityDetected);
        addMsg(finalISF_msg);
       
        return {sens:Math.min(720, round(profile.sens / final_ISF, 1)),msg};         // observe ISF maximum of 720(?)
    } else if ( bg_ISF > 1 ) {
        sens_modified = true;
    }
    ...
    var deltaType = 'pp';
    if (bg_off > 0) {
        addMsg(deltaType+"_ISF adaptation by-passed as average glucose < "+target_bg+"+10");
        
    } else if (glucose_status.short_avgdelta<0) {
        addMsg(deltaType+"_ISF adaptation by-passed as no rise or too short lived");
    } else { //if (deltaType == 'pp') {
        pp_ISF = 1 + Math.max(0, bg_delta * profile.pp_ISF_weight);
        addMsg("pp_ISF adaptation is", round(pp_ISF,2));
        if (pp_ISF != 1) {
            sens_modified = true;
        }
    }
    ...
    if (dura05<10) {
        addMsg("dura_ISF by-passed; bg is only "+dura05+"m at level "+avg05);
    } else if (avg05 <= target_bg) {
        addMsg("dura_ISF by-passed; avg. glucose", avg05, "below target", target_bg);
    } else {
        ...
        sens_modified = true;
        addMsg("dura_ISF adaptation is", round(dura_ISF,2), "because ISF", round(sens,1), "did not do it for", round(dura05,1),"m");
    }
    if ( sens_modified ) {
        liftISF = Math.max(dura_ISF, bg_ISF, acce_ISF, pp_ISF);
        if ( acce_ISF < 1 ) {                                                                           // 13.JAN.2022 brakes on for otherwise stronger or stable ISF
            addMsg("strongest autoISF factor", round(liftISF,2), "weakened to", round(liftISF*acce_ISF,2), "as bg decelerates already");
            liftISF = liftISF * acce_ISF;                                                               // brakes on for otherwise stronger or stable ISF
        }                                                                                               // brakes on for otherwise stronger or stable ISF
        const {final_ISF,msg:finalISF_msg} = withinISFlimits(liftISF, profile.autoISF_min, maxISFReduction, sensitivityRatio, origin_sens, profile, high_temptarget_raises_sensitivity, target_bg, normalTarget, stepActivityDetected, stepInactivityDetected);
        addMsg(finalISF_msg);
        return {sens:round(profile.sens / final_ISF, 1),msg};
    }
    console.error("----------------------------------");
    console.error("end autoISF");
    console.error("----------------------------------");
    //TODO: ELI - return {sens,data:{sens_modified,dura_ISF,bg_ISF,pp_ISF,smb_enabled,parabola_fit:{correlation:fit_corr,value:minmax_value,delta:minmax_delta,below_target}}};    
    return {sens,msg};                                                                                            // mod V14j: nothing changed
}
...
export function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime, flatBGsDetected) {
    ...
    let {sens:newSens,msg:autoISF_msg} = autoISF(sens, origin_sens, target_bg, profile, glucose_status, meal_data, currentTime, autosens_data, sensitivityRatio, loop_wanted_smb, high_temptarget_raises_sensitivity, normalTarget, stepActivityDetected, stepInactivityDetected);
    sens = newSens;
    rT.autoISF_msg = autoISF_msg;
